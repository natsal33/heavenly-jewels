...
export function loader({ request }) {
  const listingId = request.url.split("/")[4];
...
function Listing() {
  const listingId = useLoaderData();
  const listingItem = CONTENTS[listingId - 1];

  return (
    <section className="py-16 px-8">
      <div className="mx-auto container grid place-items-center grid-cols-1 md:grid-cols-2">
        <img src={listingItem["img"]} alt="pink blazer" className="h-[36rem]" />
        <div>
          <Typography className="mb-4" variant="h3">
            {listingItem["name"]}
          </Typography>
          <Typography variant="h5">{listingItem["price"]}</Typography>
